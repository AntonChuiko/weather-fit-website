...
- `de/`, `es/`, `ja/`, `ko/`, `zh-Hans/`, `zh-Hant/` — localized homepages (AI-translated, not human-reviewed). `zh-Hant` targets Taiwan, not Hong Kong.
...
- `de/index.html`, `es/index.html`, `ja/index.html`, `ko/index.html`, `zh-Hans/index.html` and `zh-Hant/index.html` are structural mirrors of the root `index.html` — same markup, same classes, same section order; only the visible copy and `alt` text are translated, and asset/href paths are prefixed with `../`.
